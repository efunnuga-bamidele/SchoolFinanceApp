 <header>
    ...
    <div class="row">
     ...
     <div class="col-sm-4">
          <label>Session</label>
         <select class="form-control" name='session1' id="session1" type="option" required>
                            ...
                            <!-- using jquery to pull data from json into select options -->
                              ...
                              $.getJSON("settings/session/table.json", function(json){
                                  $('#session1').empty();
                                  $('#session1').append($('<option>').text("Select a Session"));
                                  $.each(json, function(i, obj){
                                    $('#session1').append($('<option>').text(obj.item_name));
                                  });
                              });
                            </script>
         </div>
          ...
          <input type="submit" value="Search" name="get1" class="btn btn-success btn-block" >
          </div>
...
<div class="col-sm-12">
  ...
  $sql = "SELECT ID,TERM,SESSION, Class, SUM(replace(Sample_19,',','')) AS Sample_19 FROM RECEIPTS WHERE Term = '$term' AND Session = '$session' GROUP BY CLASS" ;
    ...
    var table = $('#example1').DataTable( {
     ...
     buttons: [
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            // Update footer
            document.getElementById('use1').innerHTML = '₦ '+pageTotal2;
