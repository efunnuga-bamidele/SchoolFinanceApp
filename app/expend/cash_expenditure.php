...
if($_GET){
  if(isset($_GET["save1"])){
    ...
    $save_query = $c_details->prepare('INSERT INTO EXP_HISTORY (xDate, Month, Term, Session, Exp_Details, Receiver_Name, Issuer_Name, Amount, Payment_Type, Bank) VALUES (:rdate,:month,:term,:session,:description,:reciever_name,:issuer_name,:amount,:payment_method,:bank)');
...
// connect to database
    ...
    $c_details = new SQlite3($database);
// query to update
    $stmt = $c_details->prepare("UPDATE EXP_HISTORY SET  `xDate` =  '".$rdate."',  `Month` =  '".$month."', `Term` =  '".$term."',  `Session` =  '".$session."', `Exp_Details` =  '".$description."' ,`Receiver_Name` =  '".$reciever_name."',  `Issuer_Name` =  '".$issuer_name."',  `Amount` =  '".$amount."',  `Payment_Type` =  '".$payment_method."',  `Bank` =  '".$bank."' WHERE `ID` = '$IDX' ");
      if($stmt->execute()){
        //redirection on successful registration
         echo("<script>location.href = '../expenditures.php';</script>");
        // die('Success');
  ...
  elseif(isset($_GET["delete1"])){
    ...
    $c_details = new SQlite3($database);
    $IDX = $_GET["idx"];
    $query = "DELETE FROM EXP_HISTORY WHERE rowid=$IDX";
  ...
  $sql = "SELECT * FROM EXP_HISTORY WHERE ID = $bid";
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET" id="form-sample">
    ...
    <div class="input-group mb-3 col-sm-6">
      <select class="form-control" name='session' id="sessioncx" type="option" required>
                            ...
                            <!-- using jquery to pull data from json into select options -->
                              ...
                              $.getJSON("../settings/session/table.json", function(json){
                                  $('#sessioncx').empty();
                                  $('#sessioncx').append($('<option>').text("<?php echo $Session; ?>"));
                                  $.each(json, function(i, obj){
                                    $('#sessioncx').append($('<option>').text(obj.item_name));
                                  });
                              });
                            </script>
      <div class="input-group-append">
    ...
    <div class="input-group mb-3 col-sm-12">
     <div class="input-group-append">
      ...
      <input type="text" class="form-control" placeholder="Enter issuer name" id="issuer_name" name="issuer_name" value="<?php echo $Issuer_name; ?>">
    ...
    <div class="input-group mb-3 col-sm-6">
     <div class="input-group-append">
      ...
      <input type="text" class="form-control" placeholder="Enter amount issued" id="amount" name="amount" value="<?php echo $Amount; ?>">
    ...
    <button type="submit" name="edit1" class="btn btn-secondary" disabled>Update</button>
...
<script type="text/javascript">
 $('#amount').blur(function() {
        ...
        $("#amount").maskMoney({prefix:'', allowNegative: true, thousands:',',decimal:'',precision: 0, affixesStay: false});
